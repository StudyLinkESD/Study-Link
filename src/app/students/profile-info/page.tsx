...
function StudentProfileContent() {
    ...
    if (id) {
      ...
      try {
        ...
        if (studentData) {
          ...
          // Utiliser les expériences structurées si disponibles
  ...
  // Mettre à jour le champ previousCompanies lorsque les expériences changent
  useEffect(() => {
  ...
  const handlePhotoUpload = async (file: File | null, url?: string) => {
    if (file) {
      try {
        ...
        if (url) {
          console.log('URL de photo fournie directement:', url);
          setPhotoUrl(url);
          ...
          if (session?.user?.id) {
            ...
            if (session && session.user) {
              ...
              // Forcer un rafraîchissement de la session sans recharger la page
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      if (studentId) {
        ...
        // Mettre à jour les expériences structurées
        try {
          // Convertir les expériences au format DTO
          ...
          // Envoyer les expériences à l'API
          await updateStudentExperiences(studentId, experiencesDTO);
          console.log('Expériences mises à jour avec succès:', experiencesDTO);
        } catch (error) {
        ...
        if (result && result.id) {
          ...
          // Ajouter les expériences pour le nouvel étudiant
          try {
            // Convertir les expériences au format DTO
            const experiencesDTO: ExperienceDTO[] = experiences.map((exp) => ({
              id: exp.id,
            ...
            console.log('Expériences créées avec succès:', experiencesDTO);
          } catch (error) {
                  ...
                  <div className="flex justify-between">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        console.log('Formulaire en cours de soumission:', isSubmitting);
                        ...
                        form.trigger().then((isValid) => {
                          ...
                          if (isValid) {
                            ...
                            // Afficher un message d'erreur pour aider l'utilisateur
                            toast.error(
