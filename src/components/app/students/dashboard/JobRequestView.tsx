...
export default function JobRequestView({
  ...
  const { selectedJobRequest, setSelectedJobRequest } = useJobRequest();

  const updateJobRequestStatus = async (newStatus: JobRequestStatus) => {
    if (!selectedJobRequest) return;

    try {
      const response = await axios.put(
        `/api/job-requests/${selectedJobRequest.id}`,
        { status: newStatus },
        {
          headers: {
      ...
      if (response.status >= 400) throw new Error('Failed to update job request status');

      const updatedJobRequests = jobRequests.map((req) =>
        req.id === selectedJobRequest.id ? { ...req, status: newStatus } : req,
      );

      setJobRequests(updatedJobRequests);
      setSelectedJobRequest({ ...selectedJobRequest, status: newStatus });
  ...
  const { student, job, status, createdAt, updatedAt } = selectedJobRequest;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <Select
              defaultValue={status}
              onValueChange={(value) => updateJobRequestStatus(value as JobRequestStatus)}
            >
